...
export default class TareaRepository extends TareasDataSource {
  ...
  eliminar(id) {
    try {
      //convertimos el array de tareas a una cadea jSON
      const data = JSON.stringify(this.tareas, null, 2);
      //cuardar la cadena JASONen ela rchivo
      fs.writeFileSync(filepath, data, "utf-8");
    } catch (error) {
      //si ocurre un erar al guardar los datos
      console.error("Error al eliminar la tarea:", error);
    }
  }
  //implementyarcion del eyodo eliminar
  eliinar(id) {
    try {
      ...
      console.log("error al eleimiant tarea:", error);
    }
  }
}
